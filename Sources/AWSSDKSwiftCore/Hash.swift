...
public func sha256(_ data: Data) -> [UInt8] {
    return data.withUnsafeBytes {(bytes: UnsafeRawBufferPointer) in
        var hash = [UInt8](repeating: 0, count: Int(SHA256_DIGEST_LENGTH))
        CNIOBoringSSL_SHA256(bytes.baseAddress?.assumingMemoryBound(to: UInt8.self), data.count, &hash)
        return hash
    }
}
...
public func md5(_ data: Data) -> [UInt8] {
    return data.withUnsafeBytes {(bytes: UnsafeRawBufferPointer) in
        ...
        CNIOBoringSSL_MD5(bytes.baseAddress?.assumingMemoryBound(to: UInt8.self), data.count, &hash)
        return hash
    }
}
